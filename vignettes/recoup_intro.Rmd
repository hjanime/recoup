...
%\VignetteIndexEntry{Introduction to recoup package}
-->
...
* The ChIP-Seq coverage profile vignette [here](recoup_chipseq.html)
* The RNA-Seq coverage profile vignette [here](recoup_rnaseq.html)

__Important note__: When working with gene bodies, it happens very often that 
...
1. An "automatic" way using a hybrid combination of the spline and 
neighborhood options below
2. Spline interpolation (R `spline` function with the default method)
3. Linear interpolation (R `approx` function with the default method)
4. A method based on averaging neighboring coverage points. In this case, a 
number of `NA` values are distributed randomly across the small area coverage 
vector (e.g. gene with length smaller than the desired number of bins), 
excluding the first and the last two positions, in order to reach the desired 
number of bins. Then, each `NA` position is filled with the mean value of the 
two values before and the two values after the `NA` position. This method 
should be avoided when >20% of the values of the extended vector are `NA`'s as
it may cause a crash. However, it should be the most accurate one in the 
opposite case (few `NA`'s)
See the `recoup` man page for further information.
